...
const App: React.FC = () => {
  ...
  const handleSaveApiKey = async (key: string) => {
    await Backend.auth.saveApiKey(key);
                  ...
                  <div className="service-command-center bg-[#0F1629] border border-slate-700/50 rounded-2xl overflow-hidden shadow-2xl shadow-black/50 backdrop-blur-sm relative">
                    <div className="grid grid-cols-1 md:grid-cols-2 border-b border-slate-800">
                      <button
                        ...
                        className={`p-6 text-left transition-all border-r border-slate-800 relative ${selectedTier === 'startup' ? 'bg-slate-800/50' : 'bg-transparent hover:bg-slate-800/30'
                          }`}
                      >
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className={`p-6 text-left transition-all relative ${selectedTier === 'enterprise' ? 'bg-slate-800/50' : 'bg-transparent hover:bg-slate-800/30'
                          }`}
                      >
                        <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="border-b border-slate-800 p-4 bg-[#131b2e] flex flex-col md:flex-row justify-between items-center gap-4">
                      <div className="flex bg-slate-950 rounded-lg p-1 border border-slate-800">
                        <button
                          ...
                          className={`px-5 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${inputMode === 'upload'
                        ...
                        <button
                          ...
                          className={`px-5 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${inputMode === 'github'
                              ? 'bg-slate-800 text-white shadow-sm border border-slate-700'
                              : 'text-slate-400 hover:text-slate-200'
                            }`}
                        >
                          ...
                          {(Object.keys(LANG_CONFIG) as OutputLanguage[]).map((lang) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1.5 rounded text-sm font-medium transition-all flex items-center gap-2 ${language === lang
                        ...
                        <div className="mt-8 flex flex-col items-center animate-fade-in">
                          ...
                          <div className="flex gap-4">
                            <button
                              ...
                              disabled={isProcessing}
                              className={`group relative px-8 py-4 rounded-xl font-bold text-white shadow-lg transition-all transform hover:-translate-y-1 hover:shadow-indigo-500/25 overflow-hidden ${isProcessing
                      ...
                      <div key={session.id} className="p-5 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800 transition-colors flex items-center justify-between group">
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${session.tier === 'enterprise'
